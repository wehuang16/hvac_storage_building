...
block PCM_48C_Theoretical_block_WithWaterVolume "Tes storage implementation"
...
parameter Modelica.Units.SI.SpecificHeatCapacity cPCM = 3150 "Specific heat capacity of warm PCM" annotation(Dialog(group="Material Properties"));
  ...
  Buildings.Fluid.MixingVolumes.MixingVolume vol1(
    ...
    V=0.002,
    nPorts=3)
    annotation (Placement(transformation(extent={{-158,32},{-138,52}})));
  Buildings.Fluid.MixingVolumes.MixingVolume vol2(
    redeclare package Medium = Medium,
    m_flow_nominal=mPCM_flow_nominal,
    V=0.002,
    nPorts=3) annotation (Placement(transformation(extent={{-72,30},{-52,50}})));
  Buildings.HeatTransfer.Convection.Interior con1(
    ...
    annotation (Placement(transformation(
        ...
        rotation=180,
        origin={-124,76})));
  Buildings.HeatTransfer.Convection.Interior con2(
    A=1,
    hFixed=2.5,
    til=0)
    annotation (Placement(transformation(
...
equation
  ...
  connect(senTemPCM1.port_b, vol1.ports[1]) annotation (Line(points={{-166,9},{
          -160,9},{-160,32},{-149.333,32}}, color={0,127,255}));
  connect(vol1.ports[2], coilRegisterFourPortPCM.port_a1) annotation (Line(
        points={{-148,32},{-134,32},{-134,16.2},{-116,16.2}}, color={0,127,255}));
  connect(vol1.ports[3], coilRegisterFourPortPCM.port_a2) annotation (Line(
        points={{-146.667,32},{-142,32},{-142,-8},{-78,-8},{-78,7.8},{-96,7.8}},
        color={0,127,255}));
  connect(vol2.ports[1], senTemPCM2.port_a) annotation (Line(points={{-63.3333,
          30},{-32,30},{-32,6},{0,6}}, color={0,127,255}));
  connect(vol2.ports[2], coilRegisterFourPortPCM.port_b1) annotation (Line(
        points={{-62,30},{-80,30},{-80,16.2},{-96,16.2}}, color={0,127,255}));
  connect(vol2.ports[3], coilRegisterFourPortPCM.port_b2) annotation (Line(
        points={{-60.6667,30},{-92,30},{-92,34},{-128,34},{-128,7.8},{-116,7.8}},
        color={0,127,255}));
  connect(con1.fluid, vol1.heatPort) annotation (Line(points={{-134,76},{-146,
          76},{-146,42},{-158,42}}, color={191,0,0}));
  connect(con2.fluid, vol2.heatPort) annotation (Line(points={{-62,78},{-64,78},
          {-64,40},{-72,40}}, color={191,0,0}));
  connect(con2.solid, coilRegisterFourPortPCM.heaPorOutside) annotation (Line(
        points={{-82,78},{-86,78},{-86,-4},{-95.4,-4},{-95.4,0.2}}, color={191,
          0,0}));
  connect(con1.solid, coilRegisterFourPortPCM.heaPorOutside) annotation (Line(
        points={{-114,76},{-110,76},{-110,-28},{-95.4,-28},{-95.4,0.2}}, color=
          {191,0,0}));
...
end PCM_48C_Theoretical_block_WithWaterVolume;
